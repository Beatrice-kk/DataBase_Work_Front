<template>
    <Navigation>
        ...
        <el-row
            ...
            :style="{
                width: '80%',
                padding: '5px 50px' /* 使用左右内边距代替单独的 padding-left */,
                margin: '50px auto' /* 使用 margin 自动居中 */,
                position: 'relative',
            }"
        >
            <el-col :span="4">
            ...
            <el-col :span="4">
                <el-button type="primary" @click="fetchTeachersFind"
                    >搜索</el-button
                >
            </el-col>
        </el-row>
        ...
        <el-dialog title="表单弹框" v-model="dialogVisible" width="30%">
            <el-form ref="form" :model="form" :rules="rules" label-width="80px">
                ...
                <el-form-item label="号码" prop="phone_num">
                    <el-input v-model="form.phone_num"></el-input>
                </el-form-item>
                <el-form-item label="性别" prop="gender">
                    <el-input v-model="form.gender"></el-input>
                </el-form-item>
                <el-form-item label="出生年" prop="birth_year">
                    <el-input v-model.number="form.birth_year"></el-input>
                </el-form-item>
...
export default {
    ...
    methods: {
        ...
        handleClick(row) {
            ...
            this.form.phone_num = row.phone;
            this.form.gender = row.gender;
            this.form.birth_year = row.birthYear;
            this.form.job_title = row.jobTitle;
            this.form.college = row.college;
            this.dialogVisible = true;
        },
        handleDelete(row) {
            api.deleteTeacher(row.id);
            alert("删除成功");
            window.location.reload();
        },

        handleResetPassword(row) {
            api.resetTeacherPassword(row.id);
            alert("密码重置成功");
        },
        ...
        handleSizeChange(size) {
            console.log(size);
            this.pageSize = size;
            this.fetchTeachers(this.currentPage, this.pageSize);
        },
        ...
        async fetchTeachers() {
            try {
                ...
                if (response.data.code === 200) {
                    this.teacherData = response.data.data.data;
                    this.total = response.data.data.total;
                    this.teacherData = this.teacherData.map((student) => {
        ...
        async fetchTeachersFind() {
            const { name, gender, college, jobTitle } = this.filters;
            try {
                const response = await api.fetchTeachersFind(
                    this.currentPage,
                    this.pageSize,
                    name,
                    gender,
